#' plot_optM function
#'
#' Plotting the optM results - this plotting script is based on the one provided by Treemix itself - see https://speciationgenomics.github.io/Treemix/
#'
...
plot_optM <- function(input, method = "Evanno", plot = TRUE, pdf = NULL ){
  ...
  if (method == "Evanno"){
    ...
    if (!plot){
      ...
      graphics::segments(input$m - epsilon, f.means + f.sd,
                         input$m + epsilon, f.means + f.sd, col = "red")
      graphics::legend("bottom", legend = c("likelihoods +/- SD", "% variance", "99.8% threshold"), col = c("black", grDevices::rgb(255/255,0,0,89.25/255), "black"), bty = "n", pch = c(1, 19, NA), lty = c(NA, NA, "dotted"))
      ...
      plot(input$m, f.means, pch = 19, col = grDevices::rgb(255/255,0,0,89.25/255), axes = F, ann = F)
      graphics::axis(4, las = 1)
      graphics::mtext("Variance Explained", side = 4, line = 4)
