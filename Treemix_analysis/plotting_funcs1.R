#
# functions for plotting a tree - this plotting script is based on the one provided by Treemix itself - see https://speciationgenomics.github.io/Treemix/
#
...
plot_tree_internal = function(d, e, o = NA, cex = 1.2, disp = 0.005, plus = 0.005, arrow = 0.2, ybar = 0.01, scale = T, mbar = T, mse = 0.01, plotmig = T, plotnames = T, xmin = 0, lwd = 1, font = 1){
	...
	if ( !is.na(o)){
		for(i in 1:nrow(tmp)){
			...
			if(plotnames){
				#print(tmp[i,2])
				text(tmp[i,]$x+disp, tmp[i,]$y, labels = tmp[i,2], adj = 0, cex =1.2, col  = tcol, font = font)
			}
		}
	}
	else{
		if (plotnames){
		text(tmp$x+disp, tmp$y, labels = tmp[,2], adj = 0, cex = 1.2, font = font)
		}
	}
	if (scale){
	print (paste("mse", mse))
        lines(c(0, mse*10), c(ybar, ybar))
	text( 0, ybar - 0.03, lab = "10 s.e.", adj = 0, cex  = 1.0)
	...
	lines( c(mse*10, mse*10), c(ybar- 0.01, ybar+ 0.01))
        ...
        if (mbar){
                ...
                text(xma+disp, ymi, lab = "0", adj = 0, cex = 1.0)
		if ( mw >0.5){ text(xma+disp, yma, lab = "1", adj = 0, cex = 1.2)}
                else{
			text(xma+disp, yma, lab = "0.5", adj = 0, cex =1.0)
		}
		text(0, yma+0.06, lab = "Migration", adj = 0 , cex = 1.0)
		text(0, yma+0.03, lab = "weight", adj = 0 , cex = 1.0)
        }	
}
...
plot_tree = function(stem, o = NA, cex = 1, disp = 0.003, plus = 0.01, flip = vector(), arrow = 0.2, scale = T, ybar = 0.1, mbar = T, plotmig = T, plotnames = T, xmin = 0, lwd = 1, font = 1){
